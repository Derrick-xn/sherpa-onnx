    ...
    */
    ...
    private final boolean hasPositionChanged(PointerEvent oldEvent, PointerEvent newEvent) {
        ...
        for (int i = 0; i < size; i++) {
            if (!Offset.m3041equalsimpl0(oldEvent.getChanges().get(i).getPosition(), newEvent.getChanges().get(i).getPosition())) {
                return true;
            }
        }
        return false;
    ...
    public void cleanUpHits(InternalPointerEvent internalPointerEvent) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            boolean z2 = !internalPointerEvent.m4478activeHoverEvent0FcD4WY(pointerInputChange.getId());
            boolean z3 = !this.isIn;
            if ((z && z2) || (z && z3)) {
                this.pointerIds.m4646remove0FcD4WY(pointerInputChange.getId());
            }
        }
        this.isIn = false;
        this.hasExited = PointerEventType.m4498equalsimpl0(pointerEvent.getType(), PointerEventType.INSTANCE.m4503getExit7fucELk());
    }
    ...
    public boolean dispatchMainEventPass(LongSparseArray<PointerInputChange> changes, LayoutCoordinates parentCoordinates, InternalPointerEvent internalPointerEvent, boolean isInBounds) {
        ...
        while (delegatingNodePop != 0) {
            if (delegatingNodePop instanceof PointerInputModifierNode) {
                ((PointerInputModifierNode) delegatingNodePop).mo233onPointerEventH0pRuoY(pointerEvent, PointerEventPass.Initial, jMo4682getSizeYbymL2g);
            } else if ((delegatingNodePop.getKindSet() & iM4922constructorimpl) != 0 && (delegatingNodePop instanceof DelegatingNode)) {
                Modifier.Node delegate = delegatingNodePop.getDelegate();
                int i = 0;
                delegatingNodePop = delegatingNodePop;
        ...
        if (this.modifierNode.getIsAttached()) {
            ...
            int iM4922constructorimpl2 = NodeKind.m4922constructorimpl(16);
            MutableVector mutableVector2 = null;
            while (delegatingNodePop2 != 0) {
                if (delegatingNodePop2 instanceof PointerInputModifierNode) {
                    ((PointerInputModifierNode) delegatingNodePop2).mo233onPointerEventH0pRuoY(pointerEvent, PointerEventPass.Main, jMo4682getSizeYbymL2g);
                } else if ((delegatingNodePop2.getKindSet() & iM4922constructorimpl2) != 0 && (delegatingNodePop2 instanceof DelegatingNode)) {
    ...
    public boolean dispatchFinalEventPass(InternalPointerEvent internalPointerEvent) {
        ...
        if (!this.relevantChanges.isEmpty() && this.modifierNode.getIsAttached()) {
            ...
            while (delegatingNodePop != 0) {
                if (delegatingNodePop instanceof PointerInputModifierNode) {
                    ((PointerInputModifierNode) delegatingNodePop).mo233onPointerEventH0pRuoY(pointerEvent, PointerEventPass.Final, jMo4682getSizeYbymL2g);
                } else if ((delegatingNodePop.getKindSet() & iM4922constructorimpl) != 0 && (delegatingNodePop instanceof DelegatingNode)) {
                    Modifier.Node delegate = delegatingNodePop.getDelegate();
                    int i2 = 0;
                    delegatingNodePop = delegatingNodePop;
