...
public final /* data */ class BiasAbsoluteAlignment implements Alignment {
    ...
    public long mo2890alignKFBX0sM(long size, long space, LayoutDirection layoutDirection) {
        long jIntSize = IntSizeKt.IntSize(IntSize.m6018getWidthimpl(space) - IntSize.m6018getWidthimpl(size), IntSize.m6017getHeightimpl(space) - IntSize.m6017getHeightimpl(size));
        float f = 1;
        return IntOffsetKt.IntOffset(Math.round((IntSize.m6018getWidthimpl(jIntSize) / 2.0f) * (this.horizontalBias + f)), Math.round((IntSize.m6017getHeightimpl(jIntSize) / 2.0f) * (f + this.verticalBias)));
    }
