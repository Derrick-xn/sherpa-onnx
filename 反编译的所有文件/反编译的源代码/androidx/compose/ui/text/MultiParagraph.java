...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics multiParagraphIntrinsics, long j, int i, boolean z) {
        ...
        this.maxLines = i;
        if (Constraints.m5789getMinWidthimpl(j) != 0 || Constraints.m5788getMinHeightimpl(j) != 0) {
        ...
        while (i3 < size) {
            ...
            if (Constraints.m5782getHasBoundedHeightimpl(j)) {
                iM5786getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m5786getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                iM5786getMaxHeightimpl = Constraints.m5786getMaxHeightimpl(j);
            }
            Paragraph paragraphM5182Paragraph_EkL_Y = ParagraphKt.m5182Paragraph_EkL_Y(intrinsics, ConstraintsKt.Constraints$default(0, iM5787getMaxWidthimpl, 0, iM5786getMaxHeightimpl, 5, null), this.maxLines - i4, z);
            float height = f + paragraphM5182Paragraph_EkL_Y.getHeight();
            int lineCount = i4 + paragraphM5182Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(paragraphM5182Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i4, lineCount, f, height));
            if (paragraphM5182Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i3 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i4 = lineCount;
                f = height;
                break;
        ...
        this.width = Constraints.m5787getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m5161paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m3360getUnspecified0d7_KjU();
        }
        multiParagraph.m5168paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }
    ...
    public final /* synthetic */ void m5168paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo5135paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m5167paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo5134paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m5169painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        AndroidMultiParagraphDraw_androidKt.m5573drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(final int start, final int end) {
        if (start >= 0 && start <= end && end <= getAnnotatedString().getText().length()) {
            ...
            MultiParagraphKt.m5170findParagraphsByRangeSbBc2M(this.paragraphInfoList, TextRangeKt.TextRange(start, end), new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph.getPathForRange.2
                ...
                public final void invoke2(ParagraphInfo paragraphInfo) {
                    Path.CC.m3603addPathUv8p0NA$default(Path, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
                }
            });
            return Path;
        }
    ...
    public final int m5164getOffsetForPositionk4lQ0M(long position) {
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m3045getYimpl(position)));
        if (paragraphInfo.getLength() == 0) {
            return paragraphInfo.getStartIndex();
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo5131getOffsetForPositionk4lQ0M(paragraphInfo.m5179toLocalMKHz9U(position)));
    }

    /* renamed from: getRangeForRect-8-6BmAI, reason: not valid java name */
    public final long m5165getRangeForRect86BmAI(Rect rect, int granularity, TextInclusionStrategy inclusionStrategy) {
        ...
        if (this.paragraphInfoList.get(iFindParagraphByY).getBottom() >= rect.getBottom() || iFindParagraphByY == CollectionsKt.getLastIndex(this.paragraphInfoList)) {
            ...
            return ParagraphInfo.m5177toGlobalxdX6G0$default(paragraphInfo, paragraphInfo.getParagraph().mo5132getRangeForRect86BmAI(paragraphInfo.toLocal(rect), granularity, inclusionStrategy), false, 1, null);
        }
        int iFindParagraphByY2 = MultiParagraphKt.findParagraphByY(this.paragraphInfoList, rect.getBottom());
        long jM5303getZerod9O1mEE = TextRange.INSTANCE.m5303getZerod9O1mEE();
        while (TextRange.m5291equalsimpl0(jM5303getZerod9O1mEE, TextRange.INSTANCE.m5303getZerod9O1mEE()) && iFindParagraphByY <= iFindParagraphByY2) {
            ParagraphInfo paragraphInfo2 = this.paragraphInfoList.get(iFindParagraphByY);
            jM5303getZerod9O1mEE = ParagraphInfo.m5177toGlobalxdX6G0$default(paragraphInfo2, paragraphInfo2.getParagraph().mo5132getRangeForRect86BmAI(paragraphInfo2.toLocal(rect), granularity, inclusionStrategy), false, 1, null);
            iFindParagraphByY++;
        }
        if (TextRange.m5291equalsimpl0(jM5303getZerod9O1mEE, TextRange.INSTANCE.m5303getZerod9O1mEE())) {
            return TextRange.INSTANCE.m5303getZerod9O1mEE();
        }
        long jM5303getZerod9O1mEE2 = TextRange.INSTANCE.m5303getZerod9O1mEE();
        while (TextRange.m5291equalsimpl0(jM5303getZerod9O1mEE2, TextRange.INSTANCE.m5303getZerod9O1mEE()) && iFindParagraphByY <= iFindParagraphByY2) {
            ParagraphInfo paragraphInfo3 = this.paragraphInfoList.get(iFindParagraphByY2);
            jM5303getZerod9O1mEE2 = ParagraphInfo.m5177toGlobalxdX6G0$default(paragraphInfo3, paragraphInfo3.getParagraph().mo5132getRangeForRect86BmAI(paragraphInfo3.toLocal(rect), granularity, inclusionStrategy), false, 1, null);
            iFindParagraphByY2--;
        }
        return TextRange.m5291equalsimpl0(jM5303getZerod9O1mEE2, TextRange.INSTANCE.m5303getZerod9O1mEE()) ? jM5303getZerod9O1mEE : TextRangeKt.TextRange(TextRange.m5298getStartimpl(jM5303getZerod9O1mEE), TextRange.m5293getEndimpl(jM5303getZerod9O1mEE2));
    }
    ...
    public final float[] m5163fillBoundingBoxes8ffj60Q(final long range, final float[] array, int arrayStart) {
        requireIndexInRange(TextRange.m5296getMinimpl(range));
        requireIndexInRangeInclusiveEnd(TextRange.m5295getMaximpl(range));
        final Ref.IntRef intRef = new Ref.IntRef();
        intRef.element = arrayStart;
        final Ref.FloatRef floatRef = new Ref.FloatRef();
        MultiParagraphKt.m5170findParagraphsByRangeSbBc2M(this.paragraphInfoList, range, new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph$fillBoundingBoxes$1
            /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
            {
                super(1);
            }
            ...
            public final void invoke2(ParagraphInfo paragraphInfo) {
                ...
                long jTextRange = TextRangeKt.TextRange(paragraphInfo.toLocalIndex(paragraphInfo.getStartIndex() > TextRange.m5296getMinimpl(j) ? paragraphInfo.getStartIndex() : TextRange.m5296getMinimpl(j)), paragraphInfo.toLocalIndex(paragraphInfo.getEndIndex() < TextRange.m5295getMaximpl(j) ? paragraphInfo.getEndIndex() : TextRange.m5295getMaximpl(j)));
                paragraphInfo.getParagraph().mo5129fillBoundingBoxes8ffj60Q(jTextRange, fArr, intRef2.element);
                int iM5294getLengthimpl = intRef2.element + (TextRange.m5294getLengthimpl(jTextRange) * 4);
                for (int i = intRef2.element; i < iM5294getLengthimpl; i += 4) {
                    int i2 = i + 1;
                    fArr[i2] = fArr[i2] + floatRef2.element;
                    int i3 = i + 3;
                    fArr[i3] = fArr[i3] + floatRef2.element;
                }
                intRef2.element = iM5294getLengthimpl;
    ...
    public final long m5166getWordBoundaryjx7JFs(int offset) {
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(iFindParagraphByIndex);
        return paragraphInfo.m5178toGlobalxdX6G0(paragraphInfo.getParagraph().mo5133getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)), false);
    }
