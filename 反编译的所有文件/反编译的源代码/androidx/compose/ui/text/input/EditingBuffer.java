...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m5296getMinimpl(j);
        this.selectionEnd = TextRange.m5295getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int iM5296getMinimpl = TextRange.m5296getMinimpl(j);
        int iM5295getMaximpl = TextRange.m5295getMaximpl(j);
        if (iM5296getMinimpl < 0 || iM5296getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + iM5296getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM5295getMaximpl < 0 || iM5295getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + iM5295getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM5296getMinimpl <= iM5295getMaximpl) {
            return;
        }
        throw new IllegalArgumentException("Do not set reversed range: " + iM5296getMinimpl + " > " + iM5295getMaximpl);
    }
    ...
    public final TextRange m5478getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m5286boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release, reason: not valid java name */
    public final long m5479getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long jM5480updateRangeAfterDeletepWDy79M = EditingBufferKt.m5480updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m5296getMinimpl(jM5480updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m5295getMaximpl(jM5480updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long jM5480updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m5480updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m5292getCollapsedimpl(jM5480updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m5296getMinimpl(jM5480updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m5295getMaximpl(jM5480updateRangeAfterDeletepWDy79M2);
            }
        }
    }
