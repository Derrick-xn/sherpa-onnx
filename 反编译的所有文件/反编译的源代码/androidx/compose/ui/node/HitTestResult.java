...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long jM4829findBestHitDistanceptXAw2c = m4829findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m4824getDistanceimpl(jM4829findBestHitDistanceptXAw2c) < 0.0f && DistanceAndInLayer.m4826isInLayerimpl(jM4829findBestHitDistanceptXAw2c);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        return DistanceAndInLayer.m4820compareToS_HNhKs(m4829findBestHitDistanceptXAw2c(), HitTestResultKt.DistanceAndInLayer(distanceFromEdge, isInLayer)) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m4829findBestHitDistanceptXAw2c() {
        long jDistanceAndInLayer = HitTestResultKt.DistanceAndInLayer(Float.POSITIVE_INFINITY, false);
        int i = this.hitDepth + 1;
        int lastIndex = CollectionsKt.getLastIndex(this);
        if (i <= lastIndex) {
            while (true) {
                long jM4821constructorimpl = DistanceAndInLayer.m4821constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                if (DistanceAndInLayer.m4820compareToS_HNhKs(jM4821constructorimpl, jDistanceAndInLayer) < 0) {
                    jDistanceAndInLayer = jM4821constructorimpl;
                }
                if (DistanceAndInLayer.m4824getDistanceimpl(jDistanceAndInLayer) < 0.0f && DistanceAndInLayer.m4826isInLayerimpl(jDistanceAndInLayer)) {
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(hitTestResult) && DistanceAndInLayer.m4820compareToS_HNhKs(jM4829findBestHitDistanceptXAw2c, m4829findBestHitDistanceptXAw2c()) > 0) {
