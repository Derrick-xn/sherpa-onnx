...
public final class GraphicsLayerV23 implements GraphicsLayerImpl {
    ...
    public GraphicsLayerV23(View view, long j, CanvasHolder canvasHolder, CanvasDrawScope canvasDrawScope) {
        ...
        renderNodeCreate.setClipToBounds(false);
        m4008applyCompositingStrategyWpw9cng(CompositingStrategy.INSTANCE.m3974getAutoke2Ky5w());
        this.compositingStrategy = CompositingStrategy.INSTANCE.m3974getAutoke2Ky5w();
        this.blendMode = BlendMode.INSTANCE.m3250getSrcOver0nO6VwU();
        this.alpha = 1.0f;
        this.pivotOffset = Offset.INSTANCE.m3059getUnspecifiedF1C5BW0();
        this.scaleX = 1.0f;
        this.scaleY = 1.0f;
        this.ambientShadowColor = Color.INSTANCE.m3350getBlack0d7_KjU();
        this.spotShadowColor = Color.INSTANCE.m3350getBlack0d7_KjU();
    ...
    public void mo4004setCompositingStrategyWpw9cng(int i) {
        this.compositingStrategy = i;
        updateLayerProperties();
    }

    /* renamed from: applyCompositingStrategy-Wpw9cng, reason: not valid java name */
    private final void m4008applyCompositingStrategyWpw9cng(int compositingStrategy) {
        RenderNode renderNode = this.renderNode;
        if (CompositingStrategy.m3970equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m3976getOffscreenke2Ky5w())) {
            renderNode.setLayerType(2);
            renderNode.setLayerPaint(this.layerPaint);
            renderNode.setHasOverlappingRendering(true);
        } else if (CompositingStrategy.m3970equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m3975getModulateAlphake2Ky5w())) {
    ...
    public void mo4003setBlendModes9anfk8(int i) {
        if (BlendMode.m3219equalsimpl0(this.blendMode, i)) {
            return;
        }
        this.blendMode = i;
        obtainLayerPaint().setXfermode(new PorterDuffXfermode(AndroidBlendMode_androidKt.m3137toPorterDuffModes9anfk8(i)));
        updateLayerProperties();
    }

    private final boolean requiresCompositingLayer() {
        return (!CompositingStrategy.m3970equalsimpl0(getCompositingStrategy(), CompositingStrategy.INSTANCE.m3976getOffscreenke2Ky5w()) && BlendMode.m3219equalsimpl0(getBlendMode(), BlendMode.INSTANCE.m3250getSrcOver0nO6VwU()) && getColorFilter() == null) ? false : true;
    }

    private final void updateLayerProperties() {
        if (requiresCompositingLayer()) {
            m4008applyCompositingStrategyWpw9cng(CompositingStrategy.INSTANCE.m3976getOffscreenke2Ky5w());
        } else {
            m4008applyCompositingStrategyWpw9cng(getCompositingStrategy());
        }
    }
    ...
    public void mo4005setPivotOffsetk4lQ0M(long j) {
        this.pivotOffset = j;
        if (OffsetKt.m3065isUnspecifiedk4lQ0M(j)) {
            this.shouldManuallySetCenterPivot = true;
            this.renderNode.setPivotX(IntSize.m6018getWidthimpl(this.size) / 2.0f);
            this.renderNode.setPivotY(IntSize.m6017getHeightimpl(this.size) / 2.0f);
        } else {
            this.shouldManuallySetCenterPivot = false;
            this.renderNode.setPivotX(Offset.m3044getXimpl(j));
            this.renderNode.setPivotY(Offset.m3045getYimpl(j));
        }
    }
    ...
    public void mo4002setAmbientShadowColor8_81llA(long j) {
        if (Build.VERSION.SDK_INT >= 28) {
            this.ambientShadowColor = j;
            RenderNodeVerificationHelper28.INSTANCE.setAmbientShadowColor(this.renderNode, ColorKt.m3378toArgb8_81llA(j));
        }
    }
    ...
    public void mo4007setSpotShadowColor8_81llA(long j) {
        if (Build.VERSION.SDK_INT >= 28) {
            ...
            RenderNodeVerificationHelper28.INSTANCE.setSpotShadowColor(this.renderNode, ColorKt.m3378toArgb8_81llA(j));
        }
    }
    ...
    public void mo4006setPositionH0pRuoY(int x, int y, long size) {
        this.renderNode.setLeftTopRightBottom(x, y, IntSize.m6018getWidthimpl(size) + x, IntSize.m6017getHeightimpl(size) + y);
        if (IntSize.m6016equalsimpl0(this.size, size)) {
            return;
        }
        if (this.shouldManuallySetCenterPivot) {
            this.renderNode.setPivotX(IntSize.m6018getWidthimpl(size) / 2.0f);
            this.renderNode.setPivotY(IntSize.m6017getHeightimpl(size) / 2.0f);
        }
        this.size = size;
    }
    ...
    public void record(Density density, LayoutDirection layoutDirection, GraphicsLayer layer, Function1<? super DrawScope, Unit> block) {
        Canvas canvasStart = this.renderNode.start(IntSize.m6018getWidthimpl(this.size), IntSize.m6017getHeightimpl(this.size));
        try {
            ...
            drawContext.mo3814setSizeuvyYCjk(jM6030toSizeozmzZPI);
