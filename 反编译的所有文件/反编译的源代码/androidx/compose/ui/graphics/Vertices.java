...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m3378toArgb8_81llA(colors.get(i).m3334unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM3045getYimpl = Offset.m3044getXimpl(packedValue);
            } else {
                fM3045getYimpl = Offset.m3045getYimpl(packedValue);
            }
            fArr[i] = fM3045getYimpl;
        }
        return fArr;
    }
}