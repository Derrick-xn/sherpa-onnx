...
public final class PathHitTester {
    ...
    public final boolean m3617containsk4lQ0M(long position) {
        int i;
        int iLineWinding;
        if (!this.path.isEmpty() && this.bounds.m3070containsk4lQ0M(position)) {
            ...
            if (((IntervalTree) intervalTree).root != ((IntervalTree) intervalTree).terminator) {
                ...
                while (arrayList.size() > 0) {
                    ...
                    if (node.overlaps(fM3045getYimpl, fM3045getYimpl)) {
                        T data = node.getData();
                        ...
                        if (i2 == 1) {
                            iLineWinding = BezierKt.lineWinding(points, fM3044getXimpl, fM3045getYimpl);
                        } else if (i2 == 2) {
                            iLineWinding = BezierKt.quadraticWinding(points, fM3044getXimpl, fM3045getYimpl, fArr, fArr2);
                        } else if (i2 == 3) {
                            iLineWinding = BezierKt.cubicWinding(points, fM3044getXimpl, fM3045getYimpl, fArr, fArr2);
                        }
                        i += iLineWinding;
                    }
                    if (node.getLeft() != ((IntervalTree) intervalTree).terminator && node.getLeft().getMax() >= fM3045getYimpl) {
                        arrayList.add(node.getLeft());
                    }
                    if (node.getRight() != ((IntervalTree) intervalTree).terminator && node.getRight().getMin() <= fM3045getYimpl) {
            ...
            if (PathFillType.m3611equalsimpl0(this.path.mo3177getFillTypeRgk1Os(), PathFillType.INSTANCE.m3615getEvenOddRgk1Os())) {
                i &= 1;
            }
            if (i != 0) {
                return true;
