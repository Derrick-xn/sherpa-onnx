...
public final class MonoSpline {
    ...
    public MonoSpline(float[] fArr, float[][] fArr2, float f) {
        ...
        int i2 = length - 1;
        float[][] fArrMakeFloatArray = makeFloatArray(i2, length2);
        float[][] fArrMakeFloatArray2 = makeFloatArray(length, length2);
        for (int i3 = 0; i3 < length2; i3++) {
            int i4 = 0;
            while (i4 < i2) {
                int i5 = i4 + 1;
                float f2 = fArr[i5] - fArr[i4];
                float[] fArr3 = fArrMakeFloatArray[i4];
                float f3 = (fArr2[i5][i3] - fArr2[i4][i3]) / f2;
                fArr3[i3] = f3;
                if (i4 == 0) {
                    fArrMakeFloatArray2[i4][i3] = f3;
                } else {
                    fArrMakeFloatArray2[i4][i3] = (fArrMakeFloatArray[i4 - 1][i3] + f3) * 0.5f;
                }
                i4 = i5;
            }
            fArrMakeFloatArray2[i2][i3] = fArrMakeFloatArray[length - 2][i3];
        }
        if (!Float.isNaN(f)) {
            for (int i6 = 0; i6 < length2; i6++) {
                float[] fArr4 = fArrMakeFloatArray[length - 2];
                float f4 = fArr4[i6] * (1 - f);
                float[] fArr5 = fArrMakeFloatArray[0];
                float f5 = f4 + (fArr5[i6] * f);
                fArr5[i6] = f5;
                fArr4[i6] = f5;
                fArrMakeFloatArray2[i2][i6] = f5;
                fArrMakeFloatArray2[0][i6] = f5;
            }
        }
        for (int i7 = 0; i7 < i2; i7++) {
            int i8 = 0;
            while (i8 < length2) {
                float f6 = fArrMakeFloatArray[i7][i8];
                if (f6 == 0.0f) {
                    fArrMakeFloatArray2[i7][i8] = 0.0f;
                    fArrMakeFloatArray2[i7 + 1][i8] = 0.0f;
                    i = length2;
                } else {
                    float f7 = fArrMakeFloatArray2[i7][i8] / f6;
                    int i9 = i7 + 1;
                    float f8 = fArrMakeFloatArray2[i9][i8] / f6;
                    i = length2;
                    float fHypot = (float) Math.hypot(f7, f8);
                    if (fHypot > 9.0d) {
                        float f9 = 3.0f / fHypot;
                        float[] fArr6 = fArrMakeFloatArray2[i7];
                        float[] fArr7 = fArrMakeFloatArray[i7];
                        fArr6[i8] = f7 * f9 * fArr7[i8];
                        fArrMakeFloatArray2[i9][i8] = f9 * f8 * fArr7[i8];
                    }
                }
                i8++;
                length2 = i;
        ...
        while (i7 < i6) {
            ...
            if (time < f3) {
                ...
                for (int i10 = 0; i10 < length2; i10++) {
                    ...
                    float[][] fArr4 = this.tangents;
                    v.set$animation_core_release(i10, interpolate(f5, f6, f7, f8, fArr4[i7][i10], fArr4[i9][i10]));
                }
                return;
            }
            i7 = i9;
        ...
        while (i2 < i) {
            ...
            if (f <= f3) {
                ...
                for (int i4 = 0; i4 < length2; i4++) {
                    ...
                    float[][] fArr4 = this.tangents;
                    v[i4] = diff(f5, f6, f7, f8, fArr4[i2][i4], fArr4[i3][i4]) / f5;
                }
                return;
            }
            i2 = i3;
        ...
        while (i4 < i2) {
            ...
            if (time <= f) {
                ...
                for (int i6 = 0; i6 < length2; i6++) {
                    ...
                    float[][] fArr4 = this.tangents;
                    v.set$animation_core_release(i6, diff(f3, f4, f5, f6, fArr4[i4][i6], fArr4[i5][i6]) / f3);
                }
                return;
            }
            i4 = i5;
