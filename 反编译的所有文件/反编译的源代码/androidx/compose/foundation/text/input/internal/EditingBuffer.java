...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m5298getStartimpl(j);
        this.selectionEnd = TextRange.m5293getEndimpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        checkRange(TextRange.m5298getStartimpl(j), TextRange.m5293getEndimpl(j));
    }
    ...
    public final TextRange m1178getCompositionMzsxiRA() {
        if (hasComposition()) {
    ...
    public final long m1179getSelectiond9O1mEE() {
    ...
    public final void delete(int start, int end) {
        ...
        PartialGapBuffer.replace$default(this.gapBuffer, TextRange.m5296getMinimpl(jTextRange), TextRange.m5295getMaximpl(jTextRange), "", 0, 0, 24, null);
        long jM1181updateRangeAfterDeletepWDy79M = EditingBufferKt.m1181updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m5298getStartimpl(jM1181updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m5293getEndimpl(jM1181updateRangeAfterDeletepWDy79M));
        if (hasComposition()) {
            long jM1181updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m1181updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m5292getCollapsedimpl(jM1181updateRangeAfterDeletepWDy79M2)) {
                commitComposition();
            } else {
                this.compositionStart = TextRange.m5296getMinimpl(jM1181updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m5295getMaximpl(jM1181updateRangeAfterDeletepWDy79M2);
            }
        }
        this.highlight = null;
    }
    ...
    public final void m1180setHighlightK7f2yys(int type, int start, int end) {
        if (start >= end) {
            throw new IllegalArgumentException("Do not set reversed or empty range: " + start + " > " + end);
        }
        this.highlight = new Pair<>(TextHighlightType.m1150boximpl(type), TextRange.m5286boximpl(TextRangeKt.TextRange(RangesKt.coerceIn(start, 0, getLength()), RangesKt.coerceIn(end, 0, getLength()))));
    }
