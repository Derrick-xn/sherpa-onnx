...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m5294getLengthimpl(textFieldBuffer.getChanges().mo1143getRangejx7JFs(0)) != 1 || TextRange.m5294getLengthimpl(textFieldBuffer.getChanges().mo1142getOriginalRangejx7JFs(0)) != 0 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int iM5296getMinimpl = TextRange.m5296getMinimpl(textFieldBuffer.getChanges().mo1143getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation_release() != iM5296getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(iM5296getMinimpl);
        }
    }
