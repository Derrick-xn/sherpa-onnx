...
public final class TextFieldDelegate {
    ...
    public static final class Companion {
        ...
        public static /* synthetic */ Triple m1102layout_EkL_Y$foundation_release$default(Companion companion, TextDelegate textDelegate, long j, LayoutDirection layoutDirection, TextLayoutResult textLayoutResult, int i, Object obj) {
            if ((i & 8) != 0) {
                textLayoutResult = null;
            }
            return companion.m1105layout_EkL_Y$foundation_release(textDelegate, j, layoutDirection, textLayoutResult);
        }

        @JvmStatic
        /* renamed from: layout-_EkL_-Y$foundation_release, reason: not valid java name */
        public final Triple<Integer, Integer, TextLayoutResult> m1105layout_EkL_Y$foundation_release(TextDelegate textDelegate, long constraints, LayoutDirection layoutDirection, TextLayoutResult prevResultText) {
            TextLayoutResult textLayoutResultM1092layoutNN6EwU = textDelegate.m1092layoutNN6EwU(constraints, layoutDirection, prevResultText);
            return new Triple<>(Integer.valueOf(IntSize.m6018getWidthimpl(textLayoutResultM1092layoutNN6EwU.getSize())), Integer.valueOf(IntSize.m6017getHeightimpl(textLayoutResultM1092layoutNN6EwU.getSize())), textLayoutResultM1092layoutNN6EwU);
        }

        @JvmStatic
        /* renamed from: draw-Q1vqE60$foundation_release, reason: not valid java name */
        public final void m1104drawQ1vqE60$foundation_release(Canvas canvas, TextFieldValue value, long selectionPreviewHighlightRange, long deletionPreviewHighlightRange, OffsetMapping offsetMapping, TextLayoutResult textLayoutResult, Paint highlightPaint, long selectionBackgroundColor) {
            if (!TextRange.m5292getCollapsedimpl(selectionPreviewHighlightRange)) {
                highlightPaint.mo3165setColor8_81llA(selectionBackgroundColor);
                m1101drawHighlightLepunE(canvas, selectionPreviewHighlightRange, offsetMapping, textLayoutResult, highlightPaint);
            } else if (!TextRange.m5292getCollapsedimpl(deletionPreviewHighlightRange)) {
                Color colorM3314boximpl = Color.m3314boximpl(textLayoutResult.getLayoutInput().getStyle().m5328getColor0d7_KjU());
                if (colorM3314boximpl.m3334unboximpl() == 16) {
                    colorM3314boximpl = null;
                }
                long jM3334unboximpl = colorM3314boximpl != null ? colorM3314boximpl.m3334unboximpl() : Color.INSTANCE.m3350getBlack0d7_KjU();
                highlightPaint.mo3165setColor8_81llA(Color.m3323copywmQWz5c$default(jM3334unboximpl, Color.m3326getAlphaimpl(jM3334unboximpl) * 0.2f, 0.0f, 0.0f, 0.0f, 14, null));
                m1101drawHighlightLepunE(canvas, deletionPreviewHighlightRange, offsetMapping, textLayoutResult, highlightPaint);
            } else if (!TextRange.m5292getCollapsedimpl(value.getSelection())) {
                highlightPaint.mo3165setColor8_81llA(selectionBackgroundColor);
                m1101drawHighlightLepunE(canvas, value.getSelection(), offsetMapping, textLayoutResult, highlightPaint);
            }
            TextPainter.INSTANCE.paint(canvas, textLayoutResult);
        }

        /* renamed from: drawHighlight-Le-punE, reason: not valid java name */
        private final void m1101drawHighlightLepunE(Canvas canvas, long range, OffsetMapping offsetMapping, TextLayoutResult textLayoutResult, Paint paint) {
            int iOriginalToTransformed = offsetMapping.originalToTransformed(TextRange.m5296getMinimpl(range));
            int iOriginalToTransformed2 = offsetMapping.originalToTransformed(TextRange.m5295getMaximpl(range));
        ...
        public final void notifyFocusedRect$foundation_release(TextFieldValue value, TextDelegate textDelegate, TextLayoutResult textLayoutResult, LayoutCoordinates layoutCoordinates, TextInputSession textInputSession, boolean hasFocus, OffsetMapping offsetMapping) {
            Rect rect;
            if (hasFocus) {
                int iOriginalToTransformed = offsetMapping.originalToTransformed(TextRange.m5295getMaximpl(value.getSelection()));
                ...
                long jMo4685localToRootMKHz9U = layoutCoordinates.mo4685localToRootMKHz9U(OffsetKt.Offset(rect.getLeft(), rect.getTop()));
                textInputSession.notifyFocusedRect(RectKt.m3084Recttz77jQw(OffsetKt.Offset(Offset.m3044getXimpl(jMo4685localToRootMKHz9U), Offset.m3045getYimpl(jMo4685localToRootMKHz9U)), SizeKt.Size(rect.getWidth(), rect.getHeight())));
            }
        }
        ...
        public final void updateTextLayoutResult$foundation_release(TextInputSession textInputSession, TextFieldValue textFieldValue, OffsetMapping offsetMapping, TextLayoutResultProxy textLayoutResult) {
            ...
            textInputSession.updateTextLayoutResult(textFieldValue, offsetMapping, textLayoutResult.getValue(), new Function1<Matrix, Unit>() { // from class: androidx.compose.foundation.text.TextFieldDelegate$Companion$updateTextLayoutResult$1$1$1
                ...
                public /* bridge */ /* synthetic */ Unit invoke(Matrix matrix) {
                    m1107invoke58bKbWc(matrix.m3584unboximpl());
                    return Unit.INSTANCE;
                }

                /* renamed from: invoke-58bKbWc, reason: not valid java name */
                public final void m1107invoke58bKbWc(float[] fArr) {
                    if (innerTextFieldCoordinates.isAttached()) {
                        LayoutCoordinatesKt.findRootCoordinates(innerTextFieldCoordinates).mo4689transformFromEL8BTi8(innerTextFieldCoordinates, fArr);
                    }
                }
        ...
        public final void m1106setCursorOffsetULxng0E$foundation_release(long position, TextLayoutResultProxy textLayoutResult, EditProcessor editProcessor, OffsetMapping offsetMapping, Function1<? super TextFieldValue, Unit> onValueChange) {
            onValueChange.invoke(TextFieldValue.m5561copy3r_uNRQ$default(editProcessor.toTextFieldValue(), (AnnotatedString) null, TextRangeKt.TextRange(offsetMapping.transformedToOriginal(TextLayoutResultProxy.m1127getOffsetForPosition3MmeM6k$default(textLayoutResult, position, false, 2, null))), (TextRange) null, 5, (Object) null));
        }
        ...
        public final void onBlur$foundation_release(TextInputSession textInputSession, EditProcessor editProcessor, Function1<? super TextFieldValue, Unit> onValueChange) {
            onValueChange.invoke(TextFieldValue.m5561copy3r_uNRQ$default(editProcessor.toTextFieldValue(), (AnnotatedString) null, 0L, (TextRange) null, 3, (Object) null));
            textInputSession.dispose();
        }

        /* renamed from: applyCompositionDecoration-72CqOWE, reason: not valid java name */
        public final TransformedText m1103applyCompositionDecoration72CqOWE(long compositionRange, TransformedText transformed) {
            int iOriginalToTransformed = transformed.getOffsetMapping().originalToTransformed(TextRange.m5298getStartimpl(compositionRange));
            int iOriginalToTransformed2 = transformed.getOffsetMapping().originalToTransformed(TextRange.m5293getEndimpl(compositionRange));
