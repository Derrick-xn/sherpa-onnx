...
public final class SelectionManagerKt {
    ...
    public static final Rect getSelectedRegionRect(List<? extends Pair<? extends Selectable, Selection>> list, LayoutCoordinates layoutCoordinates) {
        ...
        while (i2 < size) {
            ...
            if (offset == offset2 || (layoutCoordinates2 = selectableComponent1.getLayoutCoordinates()) == null) {
                ...
                long jMo4683localPositionOfR5De75A = layoutCoordinates.mo4683localPositionOfR5De75A(layoutCoordinates2, jOffset);
                long jMo4683localPositionOfR5De75A2 = layoutCoordinates.mo4683localPositionOfR5De75A(layoutCoordinates2, jOffset2);
                left = Math.min(left, Offset.m3044getXimpl(jMo4683localPositionOfR5De75A));
                top = Math.min(top, Offset.m3045getYimpl(jMo4683localPositionOfR5De75A));
                right = Math.max(right, Offset.m3044getXimpl(jMo4683localPositionOfR5De75A2));
                bottom = Math.max(bottom, Offset.m3045getYimpl(jMo4683localPositionOfR5De75A2));
            }
            i2++;
            size = i;
            c = 0;
    ...
    public static final long m1416calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager selectionManager, long j) {
        ...
        if (i == 1) {
            return m1418getMagnifierCenterJVtK1S4(selectionManager, j, selection.getStart());
        }
        if (i == 2) {
            return m1418getMagnifierCenterJVtK1S4(selectionManager, j, selection.getEnd());
        }
        if (i == 3) {
    ...
    private static final long m1418getMagnifierCenterJVtK1S4(SelectionManager selectionManager, long j, Selection.AnchorInfo anchorInfo) {
        ...
        if (anchorSelectable$foundation_release != null && (containerLayoutCoordinates = selectionManager.getContainerLayoutCoordinates()) != null && (layoutCoordinates = anchorSelectable$foundation_release.getLayoutCoordinates()) != null && (offset = anchorInfo.getOffset()) <= anchorSelectable$foundation_release.getLastVisibleOffset()) {
            Offset offsetM1405getCurrentDragPosition_m7T9E = selectionManager.m1405getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(offsetM1405getCurrentDragPosition_m7T9E);
            float fM3044getXimpl = Offset.m3044getXimpl(layoutCoordinates.mo4683localPositionOfR5De75A(containerLayoutCoordinates, offsetM1405getCurrentDragPosition_m7T9E.getPackedValue()));
            long jMo1366getRangeOfLineContainingjx7JFs = anchorSelectable$foundation_release.mo1366getRangeOfLineContainingjx7JFs(offset);
            if (TextRange.m5292getCollapsedimpl(jMo1366getRangeOfLineContainingjx7JFs)) {
                fCoerceIn = anchorSelectable$foundation_release.getLineLeft(offset);
            } else {
                float lineLeft = anchorSelectable$foundation_release.getLineLeft(TextRange.m5298getStartimpl(jMo1366getRangeOfLineContainingjx7JFs));
                float lineRight = anchorSelectable$foundation_release.getLineRight(TextRange.m5293getEndimpl(jMo1366getRangeOfLineContainingjx7JFs) - 1);
                fCoerceIn = RangesKt.coerceIn(fM3044getXimpl, Math.min(lineLeft, lineRight), Math.max(lineLeft, lineRight));
            }
            if (fCoerceIn == -1.0f) {
                return Offset.INSTANCE.m3059getUnspecifiedF1C5BW0();
            }
            ...
            return containerLayoutCoordinates.mo4683localPositionOfR5De75A(layoutCoordinates, OffsetKt.Offset(fCoerceIn, centerYForOffset));
        }
        return Offset.INSTANCE.m3059getUnspecifiedF1C5BW0();
    }
    ...
    public static final Rect visibleBounds(LayoutCoordinates layoutCoordinates) {
        ...
        return RectKt.m3082Rect0a9Yr6o(layoutCoordinates.mo4691windowToLocalMKHz9U(rectBoundsInWindow.m3079getTopLeftF1C5BW0()), layoutCoordinates.mo4691windowToLocalMKHz9U(rectBoundsInWindow.m3073getBottomRightF1C5BW0()));
    }

    /* renamed from: containsInclusive-Uv8p0NA, reason: not valid java name */
    public static final boolean m1417containsInclusiveUv8p0NA(Rect rect, long j) {
        float left = rect.getLeft();
        float right = rect.getRight();
        float fM3044getXimpl = Offset.m3044getXimpl(j);
        if (left <= fM3044getXimpl && fM3044getXimpl <= right) {
            float top = rect.getTop();
            float bottom = rect.getBottom();
            float fM3045getYimpl = Offset.m3045getYimpl(j);
            if (top <= fM3045getYimpl && fM3045getYimpl <= bottom) {
                return true;
            }
        }
        return false;
