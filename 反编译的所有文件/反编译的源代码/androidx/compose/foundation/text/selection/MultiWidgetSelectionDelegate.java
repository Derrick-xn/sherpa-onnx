...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (!textLayoutResult.getDidOverflowHeight() || textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                ...
                int iCoerceAtMost = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m6017getHeightimpl(textLayoutResult.getSize())), textLayoutResult.getLineCount() - 1);
                while (iCoerceAtMost >= 0 && textLayoutResult.getLineTop(iCoerceAtMost) >= IntSize.m6017getHeightimpl(textLayoutResult.getSize())) {
                    iCoerceAtMost--;
                }
    ...
    public void appendSelectableInfoToBuilder(SelectionLayoutBuilder builder) {
        ...
        long jM3048minusMKHz9U;
        LayoutCoordinates layoutCoordinates = getLayoutCoordinates();
        if (layoutCoordinates == null || (textLayoutResultInvoke = this.layoutResultCallback.invoke()) == null) {
            return;
        }
        long jMo4683localPositionOfR5De75A = builder.getContainerCoordinates().mo4683localPositionOfR5De75A(layoutCoordinates, Offset.INSTANCE.m3060getZeroF1C5BW0());
        long jM3048minusMKHz9U2 = Offset.m3048minusMKHz9U(builder.getCurrentPosition(), jMo4683localPositionOfR5De75A);
        if (OffsetKt.m3065isUnspecifiedk4lQ0M(builder.getPreviousHandlePosition())) {
            jM3048minusMKHz9U = Offset.INSTANCE.m3059getUnspecifiedF1C5BW0();
        } else {
            jM3048minusMKHz9U = Offset.m3048minusMKHz9U(builder.getPreviousHandlePosition(), jMo4683localPositionOfR5De75A);
        }
        MultiWidgetSelectionDelegateKt.m1367appendSelectableInfoParwq6A(builder, textLayoutResultInvoke, jM3048minusMKHz9U2, jM3048minusMKHz9U, getSelectableId());
    }
    ...
    public long mo1365getHandlePositiondBAh8RU(Selection selection, boolean isStartHandle) {
        TextLayoutResult textLayoutResultInvoke;
    ...
    public long mo1366getRangeOfLineContainingjx7JFs(int offset) {
        TextLayoutResult textLayoutResultInvoke = this.layoutResultCallback.invoke();
        if (textLayoutResultInvoke == null) {
