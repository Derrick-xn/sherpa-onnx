...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl mo351createnHHXs2Y(View view, boolean useTextDefault, long size, float cornerRadius, float elevation, boolean clippingEnabled, Density density, float initialZoom) {
        if (useTextDefault) {
            return new PlatformMagnifierImpl(new Magnifier(view));
        }
        long jMo398toSizeXkaWNTQ = density.mo398toSizeXkaWNTQ(size);
        float fMo397toPx0680j_4 = density.mo397toPx0680j_4(cornerRadius);
        float fMo397toPx0680j_42 = density.mo397toPx0680j_4(elevation);
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (jMo398toSizeXkaWNTQ != InlineClassHelperKt.UnspecifiedPackedFloats) {
            builder.setSize(MathKt.roundToInt(Size.m3113getWidthimpl(jMo398toSizeXkaWNTQ)), MathKt.roundToInt(Size.m3110getHeightimpl(jMo398toSizeXkaWNTQ)));
        }
        if (!Float.isNaN(fMo397toPx0680j_4)) {
            builder.setCornerRadius(fMo397toPx0680j_4);
        }
        if (!Float.isNaN(fMo397toPx0680j_42)) {
            builder.setElevation(fMo397toPx0680j_42);
        }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo350updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m3063isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m3044getXimpl(sourceCenter), Offset.m3045getYimpl(sourceCenter), Offset.m3044getXimpl(magnifierCenter), Offset.m3045getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m3044getXimpl(sourceCenter), Offset.m3045getYimpl(sourceCenter));
            }
        }
    }
}