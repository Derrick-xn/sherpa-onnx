...
final class ParentSizeNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo114measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        int iM5789getMinWidthimpl = iRound != Integer.MAX_VALUE ? iRound : Constraints.m5789getMinWidthimpl(j);
        int iM5788getMinHeightimpl = iRound2 != Integer.MAX_VALUE ? iRound2 : Constraints.m5788getMinHeightimpl(j);
        if (iRound == Integer.MAX_VALUE) {
            iRound = Constraints.m5787getMaxWidthimpl(j);
        }
        if (iRound2 == Integer.MAX_VALUE) {
            iRound2 = Constraints.m5786getMaxHeightimpl(j);
        }
        final Placeable placeableMo4680measureBRTryo0 = measurable.mo4680measureBRTryo0(ConstraintsKt.Constraints(iM5789getMinWidthimpl, iRound, iM5788getMinHeightimpl, iRound2));
        return MeasureScope.CC.layout$default(measureScope, placeableMo4680measureBRTryo0.getWidth(), placeableMo4680measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.lazy.ParentSizeNode$measure$1
            {
                super(1);
            }
