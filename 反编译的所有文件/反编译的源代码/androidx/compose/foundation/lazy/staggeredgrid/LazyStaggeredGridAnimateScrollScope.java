...
public final class LazyStaggeredGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            iM5976getXimpl = IntOffset.m5977getYimpl(lazyStaggeredGridItemInfo2.mo917getOffsetnOccac());
        } else {
            iM5976getXimpl = IntOffset.m5976getXimpl(lazyStaggeredGridItemInfo2.mo917getOffsetnOccac());
        }
        return iM5976getXimpl;
    }
    ...
    private final int calculateVisibleItemsAverageSize(LazyStaggeredGridLayoutInfo layoutInfo) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            ...
            if (layoutInfo.getOrientation() == Orientation.Vertical) {
                iM6018getWidthimpl = IntSize.m6017getHeightimpl(lazyStaggeredGridItemInfo.mo918getSizeYbymL2g());
            } else {
                iM6018getWidthimpl = IntSize.m6018getWidthimpl(lazyStaggeredGridItemInfo.mo918getSizeYbymL2g());
            }
            i += iM6018getWidthimpl;
        }
