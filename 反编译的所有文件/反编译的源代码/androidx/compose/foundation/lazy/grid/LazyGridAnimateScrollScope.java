...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            iM5976getXimpl = IntOffset.m5977getYimpl(lazyGridItemInfo2.mo846getOffsetnOccac());
        } else {
            iM5976getXimpl = IntOffset.m5976getXimpl(lazyGridItemInfo2.mo846getOffsetnOccac());
        }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (iIntValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == iIntValue) {
                    if (z) {
                        iM6018getWidthimpl = IntSize.m6017getHeightimpl(visibleItemsInfo.get(i).mo847getSizeYbymL2g());
                    } else {
                        iM6018getWidthimpl = IntSize.m6018getWidthimpl(visibleItemsInfo.get(i).mo847getSizeYbymL2g());
                    }
                    iMax = Math.max(iMax, iM6018getWidthimpl);
                    i++;
                }
                i2 += iMax;
                i3++;
