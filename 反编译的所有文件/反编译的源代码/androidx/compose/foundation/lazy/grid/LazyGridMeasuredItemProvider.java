...
public abstract class LazyGridMeasuredItemProvider implements LazyLayoutMeasuredItemProvider<LazyGridMeasuredItem> {
    ...
    public abstract LazyGridMeasuredItem mo849createItemO3s9Psw(int index, Object key, Object contentType, int crossAxisSize, int mainAxisSpacing, List<? extends Placeable> placeables, long constraints, int lane, int span);
    ...
    public LazyGridMeasuredItem mo824getAndMeasurehBUhpc(int index, int lane, int span, long constraints) {
        return m854getAndMeasurem8Kt_7k(index, constraints, lane, span, this.defaultMainAxisSpacing);
    }

    /* renamed from: getAndMeasure-m8Kt_7k, reason: not valid java name */
    public final LazyGridMeasuredItem m854getAndMeasurem8Kt_7k(int index, long constraints, int lane, int span, int mainAxisSpacing) {
        ...
        List<Placeable> listMo881measure0kLqBqw = this.measureScope.mo881measure0kLqBqw(index, constraints);
        if (Constraints.m5785getHasFixedWidthimpl(constraints)) {
            iM5788getMinHeightimpl = Constraints.m5789getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m5784getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("does not have fixed height".toString());
            }
            iM5788getMinHeightimpl = Constraints.m5788getMinHeightimpl(constraints);
        }
        return mo849createItemO3s9Psw(index, key, contentType, iM5788getMinHeightimpl, mainAxisSpacing, listMo881measure0kLqBqw, constraints, lane, span);
    }
