...
public final class PagerSnapLayoutInfoProviderKt {
    ...
    public static final float dragGestureDelta(PagerState pagerState) {
        if (pagerState.getLayoutInfo().getOrientation() == Orientation.Horizontal) {
            return Offset.m3044getXimpl(pagerState.m966getUpDownDifferenceF1C5BW0$foundation_release());
        }
        return Offset.m3045getYimpl(pagerState.m966getUpDownDifferenceF1C5BW0$foundation_release());
    }
    ...
    public static final float calculateFinalSnappingBound(PagerState pagerState, LayoutDirection layoutDirection, float f, float f2, float f3, float f4) {
        ...
        if (FinalSnappingItem.m546equalsimpl0(iCalculateFinalSnappingItem, FinalSnappingItem.INSTANCE.m550getClosestItembbeMdSM())) {
            if (Math.abs(f5) > f) {
        ...
        } else if (!FinalSnappingItem.m546equalsimpl0(iCalculateFinalSnappingItem, FinalSnappingItem.INSTANCE.m551getNextItembbeMdSM())) {
            if (FinalSnappingItem.m546equalsimpl0(iCalculateFinalSnappingItem, FinalSnappingItem.INSTANCE.m552getPreviousItembbeMdSM())) {
                return f3;
            }
            return 0.0f;
        }
