...
public final class AbsoluteCutCornerShape extends CornerBasedShape {
    ...
    public Outline mo984createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, LayoutDirection layoutDirection) {
        if (topStart + topEnd + bottomStart + bottomEnd == 0.0f) {
            return new Outline.Rectangle(SizeKt.m3134toRectuvyYCjk(size));
        }
        Path Path = AndroidPath_androidKt.Path();
        Path.moveTo(0.0f, topStart);
        Path.lineTo(topStart, 0.0f);
        Path.lineTo(Size.m3113getWidthimpl(size) - topEnd, 0.0f);
        Path.lineTo(Size.m3113getWidthimpl(size), topEnd);
        Path.lineTo(Size.m3113getWidthimpl(size), Size.m3110getHeightimpl(size) - bottomEnd);
        Path.lineTo(Size.m3113getWidthimpl(size) - bottomEnd, Size.m3110getHeightimpl(size));
        Path.lineTo(bottomStart, Size.m3110getHeightimpl(size));
        Path.lineTo(0.0f, Size.m3110getHeightimpl(size) - bottomStart);
        Path.close();
