...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        wrapEllipsisInfoEllipsisInfo$foundation_layout_release.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m51getFirstimpl(wrapEllipsisInfoEllipsisInfo$foundation_layout_release.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
        return wrapEllipsisInfoEllipsisInfo$foundation_layout_release;
    }

    /* renamed from: getWrapInfo-OpUlnko, reason: not valid java name */
    public final WrapInfo m633getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        int i = totalCrossAxisSize + currentLineCrossAxisSize;
        if (nextSize == null) {
            return new WrapInfo(true, true);
        }
        if (this.overflow.getType$foundation_layout_release() != FlowLayoutOverflow.OverflowType.Visible && (lineIndex >= this.maxLines || IntIntPair.m52getSecondimpl(leftOver) - IntIntPair.m52getSecondimpl(nextSize.getPackedValue()) < 0)) {
            return new WrapInfo(true, true);
        }
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m51getFirstimpl(leftOver) - IntIntPair.m51getFirstimpl(nextSize.getPackedValue()) < 0)) {
            if (isWrappingRound) {
                return new WrapInfo(true, true);
            }
            return new WrapInfo(true, m633getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m47constructorimpl(Constraints.m5787getMaxWidthimpl(this.constraints), (IntIntPair.m52getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m44boximpl(IntIntPair.m47constructorimpl(IntIntPair.m51getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m52getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int iMax = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m52getSecondimpl(nextSize.getPackedValue()));
        IntIntPair intIntPairM638ellipsisSizeF35zmw$foundation_layout_release = isEllipsisWrap ? null : this.overflow.m638ellipsisSizeF35zmw$foundation_layout_release(nextItemHasNext, lineIndex, iMax);
        if (intIntPairM638ellipsisSizeF35zmw$foundation_layout_release != null) {
            intIntPairM638ellipsisSizeF35zmw$foundation_layout_release.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m51getFirstimpl(leftOver) - IntIntPair.m51getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m51getFirstimpl(intIntPairM638ellipsisSizeF35zmw$foundation_layout_release.getPackedValue()) < 0) {
                if (isEllipsisWrap) {
                    return new WrapInfo(true, true);
                }
                WrapInfo wrapInfoM633getWrapInfoOpUlnko = m633getWrapInfoOpUlnko(false, 0, IntIntPair.m47constructorimpl(Constraints.m5787getMaxWidthimpl(this.constraints), (IntIntPair.m52getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m52getSecondimpl(nextSize.getPackedValue()))), intIntPairM638ellipsisSizeF35zmw$foundation_layout_release, lineIndex + 1, iMax, 0, true, true);
                return new WrapInfo(wrapInfoM633getWrapInfoOpUlnko.getIsLastItemInContainer(), wrapInfoM633getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
