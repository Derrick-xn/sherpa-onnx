...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> color, State<RippleAlpha> rippleAlpha, RippleContainer rippleContainer) {
        ...
        this.rippleSize = Size.INSTANCE.m3122getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        this.rippleSize = contentDrawScope.mo3806getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            iRoundToInt = MathKt.roundToInt(RippleAnimationKt.m1454getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo3806getSizeNHjbRc()));
        } else {
            iRoundToInt = contentDrawScope.mo391roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = iRoundToInt;
        long jM3334unboximpl = this.color.getValue().m3334unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m1459drawStateLayerH2RKhps(contentDrawScope2, this.radius, jM3334unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1458updateRipplePropertiesbiQXAtU(contentDrawScope.mo3806getSizeNHjbRc(), this.rippleRadius, jM3334unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m1457addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m3334unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
