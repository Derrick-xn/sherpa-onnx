...
class NotificationCompatBuilder implements NotificationBuilderWithBuilderAccessor {
    ...
    private void addAction(NotificationCompat.Action action) {
        ...
        if (Build.VERSION.SDK_INT >= 23) {
            builderCreateBuilder = Api23Impl.createBuilder(iconCompat != null ? iconCompat.toIcon() : null, action.getTitle(), action.getActionIntent());
        } else {
            builderCreateBuilder = Api20Impl.createBuilder(iconCompat != null ? iconCompat.getResId() : 0, action.getTitle(), action.getActionIntent());
        }
